...
下载完之后，将文件解压，解压的目录不要有中文、空格等；安装目录不要太长，目录最好放到每个电脑盘的根目录。米思齐解压后的文件也是有中文的，需要更改成不带中文，打开解压后的文件夹，需要双击"一键更新.bat"文件，按照命令框提示选择安装。等安装完成，双击“Mixly.exe”打开Mixly软件，如下图：

![mixlyOpen](./picture/mixlyOpen.png ':data-align=center')

[点击下载](https://pan.baidu.com/s/1-ki5a0iOvziBn2rjeff9Tw?pwd=i2xc)我们公司提供的米思奇安装包，可以不需要一键更新，直接下载解压，运行Mixly.exe。

打开界面如下，如果主板是Arduino系列，板卡选择Arduino AVR，双击点进编程页面，如下图：

| Arduino系类板卡选择                               | Arduino ESP32板卡选择                                     | Python ESP32板卡选择                                    |
| ------------------------------------------- | ----------------------------------------------------- | --------------------------------------------------- |
| ![mixlyArduino](./picture/mixlyArduino.png) | ![mixlyArduinoESP32](./picture/mixlyArduinoESP32.png) | ![mixlyPythonESP32](./picture/mixlyPythonESP32.png) |
...
- **垃圾箱：** 用来移除不需要的积木块。
  详细介绍可以通过查看米思齐WIKI，查看方法如下：
  选择"Arduino AVR"板卡---->打开米思齐右上角设置按钮---->点击Wiki，即可查看详细说明。其他板卡是没有这个文档，但都是类似。
  ![](./picture/mixlyWiki.png)
...
**第二步：** 选择对应的用户库。点击”云端导入“，在这里导入我们的PH2.0传感器库，如果未出现库列表，请重新一键更新米思齐软件，如下图：
...
以下库为我们公司产品的本地库，仅支持Mixly2.0 RC4版本。
...
下载完本地库，解压下载好的库，点击本地导入按钮，选择对应的文件(xxxx.xml文件)，如下图：

![](./picture/mixlyInstallLocalLib.png)

## 四、常见的米思齐问题

| 序号  | 问                                               | 答                                                                                                                                                                                       |
|:---:|:-----------------------------------------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:|
| 1   | 软件白屏不显示编程模块、缺少主板的选择项等                           | 把Mixly安装路径改成D:\mixly或其他盘的根目录，文件夹不能有中文、空格、符号等特殊字符。官方文件的原解压名也不行（自带了中文符号等），移动后再运行两次米思齐安装路径下的“一键更新.bat”。                                                                                    |
...
| 5   | ![](./picture/mixlyArduioESP32BuildError_1.png) | 选择Arduino ESP32板卡时，米思齐安装路径过长，需要将米思奇的安装包放到电脑盘符的根目录下                                                                                                                                      |
| 6   | ![](./picture/mixlyArduioESP32BuildError_2.png) | 米思齐安装路径包含中文/空格等特殊字符，需要将这些字符全部去掉                                                                                                                                                         |
| 7   | ![](./picture/mixlyESP32GD5800Error.png)        | 选择Arduino ESP32板卡时，使用GD5800 MP3块报错，需要双击运行米思齐安装路径下的“一键更新.bat”文件，更新库文件。或者使用我们公司提供一个已经安装好的米思齐版本，[点击此处下载](https://pan.baidu.com/s/1-ki5a0iOvziBn2rjeff9Tw?pwd=i2xc)                         |
| 8   | ![](./picture/mixlyArduioESP32BuildError_3.png) | 编译报错，“item1”未定义，在使用这个变量或方法名的时候，和定义的保持一致。                                                                                                                                                |
| 9   | 导入第三方库时，不显示第三方库的内容                              | 1. 确定第三方是否适应当前安装的米思齐版本(询问开发商) 2. 本公司提供的库只适合Mixly2.0 RC4版本                                                                                                                               |
| 10  | Arduino Nano米思齐程序上传不成功                          | 因Arduino Nano bootloader新老版本问题导致，解决方案：1. 主板选择Arduino/Genuino Uno; 2. 主板选择Arduino Nano时，米思齐右下角会出现“配置板卡”按钮，点击该按钮，processor下拉框选择ATmega328P，如图：![](./picture/mixlyArduinoNanoProcessor.png) |
