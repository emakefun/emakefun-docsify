...
```
namespace {
constexpr uint8_t kLedPin = 13;  // LED 引脚

constexpr uint16_t kDelayMs = 1000;  // 延时时间（毫秒）
}  // namespace

void setup() {
  pinMode(kLedPin, OUTPUT);  // 设置 LED 引脚为输出模式
}

void loop() {
  digitalWrite(kLedPin, HIGH);  // 高电平，点亮 LED
  delay(kDelayMs);              // 延时 1 秒
  digitalWrite(kLedPin, LOW);   // 低电平，熄灭 LED
  delay(kDelayMs);              // 延时 1 秒
}
```
