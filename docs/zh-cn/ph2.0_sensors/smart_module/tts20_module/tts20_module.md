...
TTS20语音合成模块是基于独家语音合成算法开发的一款高流畅度、高自然度的优美人声语音合成模块。该模块方案基于新一代神经网络算法，选取优质的女声发音人，满足通用应用场景的合成播报。

模块以中文为主，同时支持数字、字母、单词、简单的中英文混读，在合成效果（可懂度、清晰度、自然度、表现力、节奏/ 停顿、语速、语调、音质、音色、理解费力程度）方面有显著的提升。 模块通过I2C协议接收待合成的文本，直接合成为语音输出；主要是面向中高端应用，为其提供一套完整的物美价廉的语音解决方案。支持纯中文（不支持繁体字，生僻字）、常规英文文本（如字母、单词、短句）以及中英文混合合成，可实现多音字、数字、数值、日期、时间、字母正确流畅的合成，播放清晰无杂音。支持的文本编码方式为**UTF-8** 编码。支持音量、语速、语调调整，合成文本、停止合成、暂停合成、恢复合成多种控制命令。
