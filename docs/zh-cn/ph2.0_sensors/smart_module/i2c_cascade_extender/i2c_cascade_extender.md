...
- 模块内部包含一个包含一个I2C多路复用器芯片（如PCA9548A或TCA9548A）。
...
本例中采用的OLED显示屏，I2C地址是固定的，因此这两块OLED屏幕不能直接同时直接连接到同一个主板上使用。但通过I2C扩展器的转接，主控板的一个I2C口就能正常的同时使用这2块OLED显示屏了。
...
<a href="resource/dfrobot_i2c_multiplexer.zip" download>点击此处下载Arduino库程序</a>
