...
MD40模块采用先进的数字信号处理技术，支持标准的AB相正交增量式编码器。通过编码电机编码器输出的两路具有90度相位差的方波脉冲信号（A相和B相），为系统提供完整的运动信息，包括位置、运动速度和运动方向。
...
MD40模块用于驱动标准的AB相霍尔式编码电机(通过霍尔传感器、磁盘及减速电机组成的可精准控制的减速直流电机)，使用时需确保AB相信号保持90°正交关系，信号电平为3.3V/5V TTL标准。模块采用6pin PH2.0间距接口 (- V B A G +)，能够驱动符合接口标准的AB相霍尔式编码电机。
