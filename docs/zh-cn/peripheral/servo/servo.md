...
![servo_structure](picture/servo-structure.png)

![servo_parts.png](picture/servo_parts.png)

## 舵机工作原理

舵机内部的控制电路接收来自信号线的PWM控制脉冲，控制电机旋转，电机带动减速齿轮组，最后传动输出至舵盘，进而使舵机转动角度并保持。控制流程如下图所示：

![servo_operation.png](picture/servo_operation.png)
...
![servo_pwm_angle.png](picture/servo_pwm_angle.png)
...
| 重量(g)       | 舵机的总重量                                                  |
| 尺寸(mm)      | 舵机的外形尺寸                                                 |
| 速度(sec/60°) | 舵机转过60°所需的时间(mm)                                            |
| 电压(V)       | 舵机工作的电压范围                                               |
| 电流电(A)     | 舵机工作时的电流                                                |
| 温度(℃)       | 舵机工作的温度范围                                               |
| 死区(us)      | 使舵机转动的最小脉冲宽度，低于该参数，舵机不转动； 停止角度的精确度，该数值越小，控制精度越高         |
| 扭矩(kg*cm)   | 舵机所能提供的最大扭矩。表示的是距离转动轴中心1cm处，舵机能够带动的重量![servo_torque.png](picture/servo_torque.png) |
...
![servo_wiring.png](picture/servo_wiring.png)

![servo_pinout.png](picture/servo_pinout.png)
...
示例程序·实现舵机0~180°来回转动功能。

```c
#include <Servo.h>  // 引用 Servo 库

namespace {
constexpr uint8_t kServoPin = 9;  // 舵机信号线引脚

Servo g_myservo;  // 舵机对象
}  // namespace

void setup() {
  Serial.begin(9600);  // 打开串口监视器，并设置波特率为 9600

  g_myservo.attach(kServoPin);  // 告诉 Arduino，舵机的信号线连在指定引脚上
  g_myservo.write(g_angle);     // 设置舵机的初始角度为 0

  Serial.println(g_myservo.attached());  // 连上则返回 1
}

void loop() {
  // 逆时针旋转，每增加 1°，延迟 15ms
  for (uint8_t angle = 0; angle <= 180; angle++) {
    g_myservo.write(angle);  // 令舵机达到指定角度
...
1. 打开库管理，安装相应库文件
2. 搜索并安装**Servo** by Michael Margolis, Arduino，版本**1.3.0**
...
<a href="zh-cn/peripheral/servo/mixly_servo_test.zip" download>点击下载Mixly舵机测试程序</a>
...
<a href="zh-cn/peripheral/servo/microbit_micropython_servo.zip" download>点击下载microbit micropython servo库</a>

<a href="zh-cn/peripheral/servo/esp32_micropython_servo.zip" download>点击下载esp32 micropython servo库</a>

## micro:bit MakeCode Servo舵机使用

[makecode自带servo库链接](https://makecode.microbit.org/reference/servo)
